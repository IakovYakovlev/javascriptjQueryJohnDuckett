...
function writeLabel(e) {                                            // Объявляем функцию
  if(!e) {                                                          // Если оъект события отсутствует
    e = window.event;                                               // Используем вариант отката для Internet Explorer 5-8
  }
  target = event.target || event?.srcElement;                       // Получаем цель события
  textEntered = e.target.value;                                     // Значение данного элемента
...
function stop(target) {                                             // Объявляем функцию
